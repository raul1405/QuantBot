...
class Config:
    ...
    # THRESHOLD OPTIMIZATION: Reverting to Sniper (0.65) to avoid bad trades.
    ml_prob_threshold_long: float = 0.65
    ml_prob_threshold_short: float = 0.65
    ml_target_horizon: int = 5
...
class AlphaEngine:
    def __init__(self, config: Config):
        ...
        if self.config.use_alpha_engine:
            ...
            # Explicitly EXCLUDING: Basic (Z_Score, ATR), CrossSectional, ContinuousRegime
            self.feature_cols = [
                # Advanced / Structural
    ...
    def add_signals_all(self, data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for sym, df in data.items():
            ...
            # Add Meta-Features
            if 'prob_margin' in probs.columns:
                df_sig['prob_margin'] = probs['prob_margin']
                df_sig['prob_entropy'] = probs['prob_entropy']
